...
use tinstar::rules::{RuleResult, evaluate_command_rules};

#[test]
fn test_force_push_blocked() {
    let config = Config::default();
    let results = evaluate_command_rules("git push --force origin main", &config);
    assert!(results.iter().any(|r| matches!(r, RuleResult::Block { rule, .. } if rule == "force-push")));
}
...
fn test_no_verify_blocked() {
    ...
    assert!(results.iter().any(|r| matches!(r, RuleResult::Block { rule, .. } if rule == "no-verify")));
}
...
fn test_reset_hard_blocked() {
    ...
    assert!(results.iter().any(|r| matches!(r, RuleResult::Block { rule, .. } if rule == "destructive-ops")));
}
...
fn test_normal_push_allowed() {
    ...
    assert!(results.iter().all(|r| !matches!(r, RuleResult::Block { .. })));
}
